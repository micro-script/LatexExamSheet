...
\begin{function}[added = 2022-07-19]{chinese/mark-position}
  \begin{ccnusyntax}[emph={[1]mark-position}]
    mark-position = (*top|bottom|left|(right)>*)
  \end{ccnusyntax}
  \env{select} 环境的标记符号相对于内容的位置。\opt{right} 表示 \opt{mark-symbol} 在内容的右侧，其余的含义同理。
\end{function}
