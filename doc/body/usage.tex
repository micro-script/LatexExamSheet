...
\begin{function}{question/show-answer}
  \begin{ccnusyntax}[emph={[1]show-answer}]
    show-answer = (*\TFF*)
  \end{ccnusyntax}
  控制 \tn{paren} 和 \tn{fillin} 中答案的显示与否。
\end{function}
...
\begin{function}{fillin/type}
  \begin{ccnusyntax}[emph={[1]type}]
    type = (*<(line)|paren>*)
  \end{ccnusyntax}
  填空命令 \tn{fillin} 的类型。\opt{line} 表示下划线，\opt{paren} 表示括号。
  \begin{latexexample}{\tn{fillin} 的两个类型以及参数使用方式}
    答案是 \fillin \par
    答案是 \fillin[answer] \par
    答案是 \fillin[type = paren][]
  \end{latexexample}
\end{function}
...
\begin{function}[added = 2022-07-02]{solution/score-style}
  \begin{ccnusyntax}[emph={[1]score-style}]
    score-style = (*\meta{风格设置}*)
  \end{ccnusyntax}
...
\begin{latexexample}{\opt{score-style} 示例}
  \examsetup{solution/score-style = {\sffamily \huge \color{blue}}}
  \begin{solution}
    测试 \score{2}
  \end{solution}
\end{latexexample}
